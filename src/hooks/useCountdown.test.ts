...
import { readAppState, writeAppState, writeStoredTimes } from "@/util/appLocalStorage";
...
describe(useCountdown, () => {
  describe("normal usage", () => {
    ...
    it.each([
      ...
      "`stop` returns to 'editing' from other phases (e.g. '%s')",
      (expectedPhase, advanceTime) => {
        const { result } = renderHook(useCountdown);
  ...
  // Phones may discard the page sometimes, reloading it looks exactly like a fresh mount
  describe("resuming a stored session", () => {
    const storeSession = (secondsFromNow: number, warningSeconds: number) => {
      writeAppState({ isRunning: true, deadlineMs: Date.now() + secondsFromNow * 1000 });
      writeStoredTimes({ sharingSeconds: 150, warningSeconds });
    };
    ...
    it.each([
      ...
      "can pick the countdown back up during the '%s' phase",
      (expectedPhase, secondsFromNow, warningSeconds) => {
        storeSession(secondsFromNow, warningSeconds);
        const { result } = renderHook(useCountdown);

        expect(result.current.countdownPhase).toBe(expectedPhase);
        expect(result.current.remainingTimerSeconds).toBe(secondsFromNow);
      },
    );

    it("keeps counting down from where it left off", () => {
      storeSession(300, 60);
      const { result } = renderHook(useCountdown);

      act(() => vi.advanceTimersByTime(10000));
      expect(result.current.remainingTimerSeconds).toBe(290);

      act(() => vi.advanceTimersByTime(230000));
      expect(result.current.countdownPhase).toBe("warning");
      expect(result.current.remainingTimerSeconds).toBe(60);
    });

    it("starts in 'editing' when the stored session is not running", () => {
    ...
    it("starts in 'editing' when the stored session is very old", () => {
      writeAppState({ isRunning: true, deadlineMs: 1700000000000 });
      const { result } = renderHook(useCountdown);
    ...
    it("`start` stores the app state to Local Storage", () => {
      const { result } = renderHook(useCountdown);
      act(() => result.current.start(300, 60));
      expect(readAppState()).toEqual({ isRunning: true, deadlineMs: Date.now() + 300000 });
    });

    it("`stop` clears the session so a reload returns to editing", () => {
      const { result } = renderHook(useCountdown);
      act(() => result.current.start(300, 60));
      expect(result.current.countdownPhase).toBe("running");
      expect(readAppState().isRunning).toBe(true);

      act(() => result.current.stop());
      expect(result.current.countdownPhase).toBe("editing");
      expect(readAppState().isRunning).toBe(false);
    });
  });
});
